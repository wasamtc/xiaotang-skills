...
### 2.2 Lint the Files to Be Added/Committed

Before **any** `git add` or `git commit`, run `lintrunner` on the exact file set confirmed in Step 1.3:

```bash
lintrunner <file1> <file2> ...
```

Rules:

1. Only lint the confirmed file list from Step 1.3. Do **not** switch to `--all-files` unless the user explicitly asks.
2. If `lintrunner` reports formatting or auto-fixable issues, immediately apply fixes on the same file set:
   ```bash
   lintrunner -a <file1> <file2> ...
   ```
3. After auto-fix, rerun the lint check on the same file set:
   ```bash
   lintrunner <file1> <file2> ...
   ```
4. If files were modified by `lintrunner -a`, review the diff and tell the user which files changed before staging.
5. If `lintrunner` fails because the environment is not initialized, run:
   ```bash
   make setup-lint
   ```
   then retry `lintrunner`.
6. If the remaining lint failures are **not** auto-fixable, stop and report them to the user. Do **not** stage or commit until the lint check passes or the user explicitly instructs otherwise.
...
Stage the post-`lintrunner` version of those files. If `lintrunner -a` changed any file, add the fixed content rather than the pre-fix version.

### 2.4 Squash into One Commit
...
- [ ] Step 2.2: Run `lintrunner` on the confirmed file set
- [ ] Step 2.3: Stage confirmed files only
- [ ] Step 2.4: Squash to exactly one commit
...
| `lintrunner` found auto-fixable issues | Run `lintrunner -a <confirmed files...>`, rerun lint, then continue only if it passes. |
| `lintrunner` found non-auto-fixable issues | Stop. Report the remaining lint errors to the user and do not stage/commit. |
