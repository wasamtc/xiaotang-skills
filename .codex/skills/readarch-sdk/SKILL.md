...
name: readarch-sdk
description: >-
  Search SIPU SDK headers under /share_data/sicx_sdk/release/latest/ for
  low-level intrinsic functions, device APIs, runtime APIs, and hardware
  instructions. ONLY use when the user explicitly types /readarch in their
  message. Do NOT trigger automatically or for any other reason. This skill
  complements the readarch skill (which reads docs/tile_extension.md for SIPU
  instruction set) — both should be invoked together on /readarch.
---

# /readarch — SIPU SDK Intrinsic & API Lookup
